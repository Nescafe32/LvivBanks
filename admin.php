...
<script>
    ...
    function create_marker(mId, mType, mName, mAddress, mStatus, mImage, mStartTime, mEndTime, isDragable, location) {
        ...
        var contentString = $('<div class="marker-info-win">' +
            ...
            '<h1 class="marker-heading">' + mName + ' ' + mType + '</h1>' + mAddress +
            '<p>' + mStartTime + '-' + mEndTime + '</p>' +
            '</span><p><button name="remove-marker" class="remove-marker" title="Remove Marker">Remove Marker</button></p></div></div>');
    ...
    function create_marker1(mType, mName, mAddress, mImage, isDragable, location) {
        var marker = new google.maps.Marker({
            map: map
            , type: mType
            , name: mName
        ...
        if (mType === null) {
            contentString = $('<div class="marker-info-win">' +
                '<div class="marker-inner-win"><span class="info-content">' +
                '<h1 class="marker-heading">' + mName + '</h1>' + mAddress +
        ...
        if (typeof saveBtn !== 'undefined') {
            google.maps.event.addDomListener(saveBtn, "click", function (event) {
                ...
                var mName = contentString.find('select.save-name')[0].value;
                var mAddress = contentString.find('textarea.save-address')[0].value;
                if (mAddress == '') {
                ...
                var tueStart = $("#tuestart").val();
                var tueEnd = $("#tueend").val();
                var wedStart = $("#wedstart").val();
                var wedEnd = $("#wedend").val();
                var thuStart = $("#thustart").val();
                var thuEnd = $("#thuend").val();
                var friStart = $("#fristart").val();
                var friEnd = $("#friend").val();
                var satStart = $("#satstart").val();
                var satEnd = $("#satend").val();
                var sunStart = $("#sunstart").val();
                var sunEnd = $("#sunend").val();

                if ((monStart == '' && monEnd != '') || (monEnd == '' && monStart != ''))
                    alert("Wrong Monday time!");
                else if ((tueStart == '' && tueEnd != '') || (tueEnd == '' && tueStart != ''))
                    alert("Wrong Tuesday time!");
                else if ((wedStart == '' && wedEnd != '') || (wedEnd == '' && wedStart != ''))
                    alert("Wrong Wednesday time!");
                else if ((thuStart == '' && thuEnd != '') || (thuEnd == '' && thuStart != ''))
                    alert("Wrong Thursday time!");
                else if ((friStart == '' && friEnd != '') || (friEnd == '' && friStart != ''))
                    alert("Wrong Friday time!");
                else if ((satStart == '' && satEnd != '') || (satEnd == '' && satStart != ''))
                    alert("Wrong Saturday time!");
                else if ((sunStart == '' && sunEnd != '') || (sunEnd == '' && sunStart != ''))
                    alert("Wrong Sunday time!");
                else {
                    var wtimes = [monStart, monEnd, tueStart, tueEnd, wedStart, wedEnd, thuStart, thuEnd, friStart, friEnd, satStart, satEnd, sunStart, sunEnd];
                    save_marker(marker, mType, mName, mAddress, mReplace, wtimes);
                }
            });
        }
        google.maps.event.addListener(marker, 'rightclick', function () {
    ...
    function remove_marker(marker) {
        if (marker.getDraggable()) {
            ...
            var myData = {
                del: 'true'
                , markerid: marker['locid']
            };
            $.ajax({
                type: "POST"
                , url: "php/markerOperations.php"
                , data: myData
    ...
    function save_marker(marker, mType, mName, mAddress, replaceWin, wtimes) {
        var geocoder = new google.maps.Geocoder();
        console.log(marker);
        geocodeAddress(geocoder, function(pos) {
            ...
            var markerData = {
                ...
                , address: mAddress
                , latlang: location
                , monStart: (wtimes[0] === "" ? null : wtimes[0] + ":00")
                , monEnd: (wtimes[1] === "" ? null : wtimes[1] + ":00")
                , tueStart: (wtimes[2] === "" ? null : wtimes[2] + ":00")
                , tueEnd: (wtimes[3] === "" ? null : wtimes[3] + ":00")
                , wedStart: (wtimes[4] === "" ? null : wtimes[4] + ":00")
                , wedEnd: (wtimes[5] === "" ? null : wtimes[5] + ":00")
                , thuStart: (wtimes[6] === "" ? null : wtimes[6] + ":00")
                , thuEnd: (wtimes[7] === "" ? null : wtimes[7] + ":00")
                , friStart: (wtimes[8] === "" ? null : wtimes[8] + ":00")
                , friEnd: (wtimes[9] === "" ? null : wtimes[9] + ":00")
                , satStart: (wtimes[10] === "" ? null : wtimes[10] + ":00")
                , satEnd: (wtimes[11] === "" ? null : wtimes[11] + ":00")
                , sunStart: (wtimes[12] === "" ? null : wtimes[12] + ":00")
                , sunEnd: (wtimes[13] === "" ? null : wtimes[13] + ":00")
            };
            $.ajax({
                type: "POST"
                , url: "php/markerOperations.php"
                ...
                , success: function (data) {
                    ...
                    marker.setIcon('images/' + mName + '.png');
                    markers.push(marker);
                }
                , error: function (xhr, ajaxOptions, thrownError) {
                    alert(thrownError);
...
//            maxZoom: 18,
        ...
        google.maps.event.addListener(lvivArea, 'click', function (event) {
            var editForm = '<p><div class="marker-edit">' +
                '<label for="pType">' +
                '<span>Type: </span><select name="pType" class="save-type"><option value="department">Department</option><option value="atm">ATM</option></select></label>' +
                '<label for="pName"><p><span>Name: </span><select name="pName" class="save-name">' +
                '<option value="abank">A-bank</option>' +
                '<option value="accordbank">Accordbank</option>' +
                '<option value="alphabank">Alphabank</option>' +
                '<option value="apeksbank">Apeksbank</option>>' +
                '<option value="bmbank">BMbank</option>' +
                '<option value="btabank">BTAbank</option>' +
                '<option value="concordbank">Concordbank</option>' +
                '<option value="creditagricolebank">Creditagricolebank</option>>' +
                '<option value="creditdniprobank">Creditdniprobank</option>' +
                '<option value="expressbank">Expressbank</option>' +
                '<option value="forwardbank">Forwardbank</option>' +
                '<option value="grantbank">Grantbank</option>>' +
                '<option value="ideabank">Ideabank</option>' +
                '<option value="industrialbank">Industrialbank</option>' +
                '<option value="kredobank">Kredobank</option>' +
                '<option value="lvivbank">Lvivbank</option>' +
                '<option value="megabank">Megabank</option>>' +
                '<option value="mibbank">MIBbank</option>' +
                '<option value="oksibank">Oksibank</option>' +
                '<option value="oschadbank">Oschadbank</option>' +
                '<option value="otpbank">OTPbank</option>>' +
                '<option value="pinbank">PINbank</option>' +
                '<option value="piraeusbank">Piraeusbank</option>' +
                '<option value="pivdenniybank">Pivdenniy</option>>' +
                '<option value="poltavabank">Poltavabank</option>' +
                '<option value="praveksbank">Praveksbank</option>' +
                '<option value="privatbank">Privatbank</option>' +
                '<option value="procreditbank">Procreditbank</option>>' +
                '<option value="prominvestbank">Prominvestbank</option>' +
                '<option value="pumbbank">PUMBbank</option>' +
                '<option value="raiffeisenbank">Raiffeisenbank</option>' +
                '<option value="sberbank">Sberbank</option>>' +
                '<option value="sichbank">Sichbank</option>' +
                '<option value="tascombank">Tascombank</option>' +
                '<option value="ukreximbank">Ukreximbank</option>' +
                '<option value="ukrgazbank">Ukrgazbank</option>>' +
                '<option value="ukrsibbank">Ukrsibbank</option>' +
                '<option value="unicreditbank">Unicreditbank</option>' +
                '<option value="universalbank">Universalbank</option>' +
                '<option value="vsbank">VSbank</option>>' +
                '<option value="vtbbank">VTBbank</option>>' +
                '</select></p></label>' +
                '<label for="pAddress"><span>Address: </span><textarea name="pAddress" id="marker-address" class="save-address" placeholder="Enter Address" maxlength="90"></textarea></label>' +
                '<p><span>Monday:</span><span style="white-space:pre">&nbsp;&nbsp;&emsp;&emsp;&emsp;&emsp;<input id="monstart" type="time" /></span><span style="white-space:pre">&emsp;&emsp;<input id="monend" type="time" /><span>' +
                '<p><span>Tuesday:</span><span style="white-space:pre">&nbsp;&nbsp;&emsp;&emsp;&emsp;&emsp;<input id="tuestart" type="time" /></span><span style="white-space:pre">&emsp;&emsp;<input id="tueend" type="time" /><span>' +
                '<p><span>Wednesday:</span><span style="white-space:pre">&emsp;&emsp;&emsp;<input id="wedstart" type="time" /></span><span style="white-space:pre">&emsp;&emsp;<input id="wedend" type="time" /><span>' +
                '<p><span>Thursday:</span><span style="white-space:pre">&emsp;&emsp;&emsp;&emsp;<input id="thustart" type="time" /></span><span style="white-space:pre">&emsp;&emsp;<input id="thuend" type="time" /><span>' +
                '<p><span>Friday:</span><span style="white-space:pre">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&emsp;&emsp;&emsp;&emsp;<input id="fristart" type="time" /></span><span style="white-space:pre">&emsp;&emsp;<input id="friend" type="time" /><span>' +
                '<p><span>Saturday:</span><span style="white-space:pre">&nbsp;&emsp;&emsp;&emsp;&emsp;<input id="satstart" type="time" /></span><span style="white-space:pre">&emsp;&emsp;<input id="satend" type="time" /><span>' +
                '<p><span>Sunday:</span><span style="white-space:pre">&nbsp;&nbsp;&nbsp;&emsp;&emsp;&emsp;&emsp;<input id="sunstart" type="time" /></span><span style="white-space:pre">&emsp;&emsp;<input id="sunend" type="time" /><span>' +
                '</div></p><button name="save-marker" class="save-marker">Save Marker Details</button>';
            create_marker1(null, 'New Bank Item', editForm, "images/bankDefault.png", true, event.latLng);
        });
    }

    function geocodeAddress(geocoder, callback) {
        var address = $("#marker-address").val();
        geocoder.geocode({
            'address': address
        }, function (results, status) {
            if (status === 'OK') {
